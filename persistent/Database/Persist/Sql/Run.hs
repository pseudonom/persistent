...
runSqlPool
    :: (MonadBaseControl IO m, HasPersistBackend backend SqlBackend)
...
runSqlConn
    ...
    => MonadBaseControl IO m => ReaderT backend m a -> backend -> m a
...
liftSqlPersistMPool
    :: (MonadIO m, HasPersistBackend backend SqlBackend)
    => ReaderT backend (NoLoggingT (ResourceT IO)) a -> Pool backend -> m a
liftSqlPersistMPool x pool = liftIO (runSqlPersistMPool x pool)

withSqlPool
    :: (MonadIO m, MonadLogger m, MonadBaseControl IO m, HasPersistBackend backend SqlBackend)
...
withSqlConn
    :: (MonadIO m, MonadBaseControl IO m, MonadLogger m, HasPersistBackend backend SqlBackend)
    => (LogFunc -> IO backend) -> (backend -> m a) -> m a
withSqlConn open f = do
    logFunc <- askLogFunc
    bracket (liftIO $ open logFunc) (liftIO . close') f

close' :: (HasPersistBackend backend SqlBackend) => backend -> IO ()
close' conn = do
