...
-- been released.
withPostgresqlPool :: (MonadBaseControl IO m, MonadLogger m, MonadIO m, IsPersistBackend SqlBackend backend)
                   => ConnectionString
...
-- control.
createPostgresqlPool :: (MonadIO m, MonadBaseControl IO m, MonadLogger m, IsPersistBackend SqlBackend backend)
                     => ConnectionString
                     -- ^ Connection string to the database.
                     -> Int
...
-- Since 2.1.3
createPostgresqlPoolModified
    :: (MonadIO m, MonadBaseControl IO m, MonadLogger m, IsPersistBackend SqlBackend backend)
...
withPostgresqlConn :: (MonadIO m, MonadBaseControl IO m, MonadLogger m, IsPersistBackend SqlBackend backend)
                   => ConnectionString -> (a -> m backend) -> m backend
withPostgresqlConn = withSqlConn . open' (const $ return ())

open'
    :: IsPersistBackend SqlBackend backend
...
openSimpleConn :: IsPersistBackend SqlBackend backend => LogFunc -> PG.Connection -> IO backend
