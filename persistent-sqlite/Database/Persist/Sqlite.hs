...
createSqlitePool
    :: (MonadIO m, MonadLogger m, MonadBaseControl IO m, IsPersistBackend SqlBackend backend)
...
withSqlitePool :: (MonadBaseControl IO m, MonadIO m, MonadLogger m, IsPersistBackend SqlBackend backend)
               => Text
               -> Int -- ^ number of connections to open
               -> (Pool backend -> m a) -> m a
withSqlitePool s = withSqlPool $ open' s

withSqliteConn :: (MonadBaseControl IO m, MonadIO m, MonadLogger m, IsPersistBackend SqlBackend backend)
               => Text -> (backend -> m a) -> m a
withSqliteConn = withSqlConn . open'

open' :: (IsPersistBackend SqlBackend backend) => Text -> LogFunc -> IO backend
...
--
-- Since 1.1.5
wrapConnection :: (IsPersistBackend SqlBackend backend) => Sqlite.Connection -> LogFunc -> IO backend
...
--
-- Since 1.1.4
runSqlite :: (MonadBaseControl IO m, MonadIO m, IsPersistBackend SqlBackend backend)
